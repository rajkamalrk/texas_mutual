...
def log_and_email(env_profile, provider, load_category, reprocess_flag, batch_id, data_pipeline_name, data_pipeline_id, error_info, error_message, severity, sns_topic_arn):
    ...
    message_body = (
        f"Process : {provider}-{load_category} - Step Function Success Lambda \n"
        f"Batch ID: {batch_id}\n"
    ...
    try:
        ...
        if isinstance(event, dict):
            ...
            if 'parsedBody' in event:
                ...
                logger.info(f'load_category=>{load_category}')
            ...
            if batch_id:
                step_id = preauth_step_logging(secrets, batch_id, step_id, lambda_name, aws_request_id, 'IN PROGRESS', 'Lambda - PreAuth Success')
                logger.info(f"step_id => {step_id}")

                update_preauth_batch_status(secrets, batch_id, status, 'Lambda - PreAuth Success')
                logger.info(f"[SUCCESS] Successfully updated batch_status_id {batch_id} to status {status}.")

                preauth_step_logging(secrets, batch_id, step_id, None, None, status, 'Lambda - PreAuth Success')
            ...
            result_body = {
                'batch_id': batch_id,
                'status': status,
                'Comment': "All Process Completed Successfully ....",
            }
        ...
        severity="FATAL"
        response=log_and_email(env_profile, provider, load_category, reprocess_flag, batch_id, data_pipeline_name, data_pipeline_id, result_body['error_info'], result_body['error_message'], severity, sns_topic_arn)
        logger.info(f"Email sent successfully!message_id:{response['MessageId']}")
        if batch_id:
            preauth_error_logging(secrets, batch_id, step_id, status, 'PREAUTH SUCCESS FLOW FAILED', result_body['error_info'], result_body['error_message'], 'Lambda - PreAuth Success')
