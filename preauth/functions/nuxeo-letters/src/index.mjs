...
export const handler = async (event) => {
    ...
    try {
        ...
        [result] = await connection.query(`
          ...
          WHERE 
            claim_number = ? and header_id = ?
        `, [claimNumber, headerId]);
                ...
                try{
                    ...
                    if (!result[i].document_serial_number){
                        ...
                        if (response.documentSerialNumber){
                            ...
                            const updateQueryBT = `UPDATE ${process.env.DB_1}.${process.env.TABLE_1} SET  update_user = 'LAMBDA', update_timestamp = ?, document_serial_number  = ?, nuxeo_status = ? WHERE claim_number = ? and header_id = ?`;
                            const updateValuesBT = [updatedOn, response.documentSerialNumber, 'COMPLETED', result[i].claim_number, result[i].header_id];
                    ...
                    const updateQueryBT = `UPDATE ${process.env.DB_1}.${process.env.TABLE_1} SET  update_user = 'LAMBDA', update_timestamp = ?, nuxeo_status = ? WHERE claim_number = ? and header_id = ?`;
                    const updateValuesBT = [updatedOn, 'FAILED', result[i].claim_number, result[i].header_id];
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                    const insertValues = [batchId, stepId, "LAMBDA FAILED", "Failed while sending request to NUXEO", JSON.stringify(error), createdOn, createdOn, 'LAMBDA', 'LAMBDA', result[i].claim_number];
                    await connection.beginTransaction();
                    await connection.query(updateQueryBT, updateValuesBT);
