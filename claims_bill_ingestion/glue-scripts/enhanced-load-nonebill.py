...
from pyspark.sql.functions import lit, col, lower, udf, count,when, expr, concat, round
...
logger = logging.getLogger("claimant-billingestion-enhanced-load-nonebill-job")
...
def get_df_db_data_with_filter_id(source, table_name, table_type, select_cols_list, filter_id_col_name, filter_id_col_value, bill_header_table_name, secrets):
    try:
        jdbc_url = f'jdbc:mysql://{secrets["host"]}:{secrets["port"]}/{secrets["dbClusterIdentifier"]}'
        if table_type == "BILL_HEADER":
            read_db_sql = f"""(SELECT {select_cols_list} FROM {table_name} WHERE source = '{source}' AND {filter_id_col_name} = {filter_id_col_value}) AS db_vw"""
        elif filter_id_col_name == "file_header_id" and table_type != "BILL_HEADER":
            read_db_sql = f"""(SELECT {select_cols_list.replace('bill_id','b.bill_id')} FROM {table_name} l JOIN {bill_header_table_name} b ON l.bill_id = b.bill_id WHERE b.source = '{source}' AND {filter_id_col_name} = {filter_id_col_value}) AS db_vw"""
        else:
...
def log_and_email(env_profile,errorInfo,file_header_id,bill_header_id,source,trading_partner,errormessage,severity,sns_topic_arn):
     ...
     #create log message
     process=f"Process : {source} - Extract, Validate and Transform Bill Metadata to load into Enhanced tables"
...
def log_and_email_attachment(process,env_profile, file_header_id,batch_id,source, trading_partner, bucket_name, source_prefix, target_prefix):
    ...
    try:
        ...
        validation_error_mail_df = validation_error_df.withColumn('Error Log ID',lit(batch_id)) \
            ...
            .withColumnRenamed('invoice_number','Trading Partner Bill ID') \
        ...
        target_file_name = f'{source_tp_str}_{file_header_id}_{process}_{timestamp}.csv'
...
## @params: [claimant-billingestion-enhanced-load-nonebill-job]
...
source=args['source']
trading_partner = args.get('trading_partner', None)  # Default to None if not present
file_header_id = args.get('file_header_id')
...
try:
    ...
    logger.info(f"db_sp_params => {db_sp_params}")
    dbStoredProcedure = "txm_bitx_enhanced_logs.insert_enhanced_nonebill_logs"
    ...
    read_db_sql = f"""
        SELECT * FROM txm_bitx_enhanced_logs.batch_non_ebill_master 
        WHERE source = '{source}'
        AND trading_partner IS NULL
    ...
    if existing_log_count > 0:
        ...
        logger.info(f"No existing entry found for Source {source}, trading partner {trading_partner} and File Header ID {file_header_id}. Proceeding with enhanced load - non ebill...")
    ...
    if not is_skip_execution:
        ...
        config_rec = AWSGlueDataCatalog_EnhancedConfig.toDF().filter(lower(col("source")) == source.lower()).first()
        logger.info(f"config_rec => {config_rec}")

        logger.info(f"input file_header_id => {file_header_id}")
        ...
        # Check if data available for Input Source, Trading Partner and File Header ID
        if bill_list_count > 0:
            logger.info(f"Data Available for Source {source}, trading partner {trading_partner} and File Header ID {file_header_id}...")
            ...
            if source.lower() == 'kofax':
                ...
                source_validation_config_df = AWSGlueDataCatalog_ValidationTPConfig.toDF()
                source_validation_config_df.show(3, truncate=False)
                validation_config_count = source_validation_config_df.count()
                logger.info(f"Trading Partner Validation config Record Count => {validation_config_count}")
                source_validation_config_itr = source_validation_config_df.orderBy(['order_seq_no']).rdd.toLocalIterator()
                ...
                for row in source_validation_config_itr:
                    ...
                    join_condition = [bill_list_df['form_type'] == validation_df['form_type'], bill_list_df['bill_header_id'] == validation_df['bill_header_id']]
                    ...
                    temp_validation_result_df = temp_validation_result_df.withColumn("validation_no",lit(row["order_seq_no"])) \
                        ...
                        .select(bill_list_df["header_id"],bill_list_df['form_type'],bill_list_df["bill_header_id"],col("field_name_1"),col("field_value_1"),col("field_name_2"),col("field_value_2"),col("invoice_number"),col("validation_no"),col("validation_type"),col("validation_result"),col("validation_error_message")) \
                        .cache()
                    ...
                    if row["validation_type"] == "primary":
                        ...
                        if primary_validation_errors_df.count() == 0:
                            ...
                            join_condition = [temp_validation_result_df['form_type'] == primary_validation_errors_df['form_type'], temp_validation_result_df['bill_header_id'] == primary_validation_errors_df['bill_header_id']]
                ...
                partition_cols = ["form_type", "bill_header_id"]
                ...
                validation_error_df = validation_result_df.filter(validation_result_df["validation_result"] == "FAIL") \
                    .select(col("form_type"),col("bill_header_id"),col("validation_error_message"),col('field_name_1'),col('field_value_1'),col('field_name_2'),col('field_value_2'),col('invoice_number')) \
                    .cache()
                ...
                if validation_error_count > 0:
                    ...
                    process=f'{source.capitalize()} Validations'
                    response = log_and_email_attachment(process,env_profile, file_header_id,batch_id, source,trading_partner, s3_bucket, s3_error_attachment_prefix, s3_error_attachment_prefix)
                ...
                logger.info(f"Validation process is skipped for Source {source}!!!")

            logger.info(f'Reading {source} Transformation configuration from Data Catalog table => {config_rec["datacatalog_schema_name"]}.{config_rec["datacatalog_transformation_table_name"]}...')
            AWSGlueDataCatalog_EnhancedSourceConfig = glueContext.create_dynamic_frame.from_catalog(database=config_rec["datacatalog_schema_name"], table_name=config_rec["datacatalog_transformation_table_name"], transformation_ctx="AWSGlueDataCatalog_EnhancedTPConfig")
            AWSGlueDataCatalog_EnhancedSourceConfig.printSchema()
            AWSGlueDataCatalog_EnhancedSourceConfig.show()
            source_transformation_config_df = AWSGlueDataCatalog_EnhancedSourceConfig.toDF()
            source_transformation_config_df.show(3, truncate=False)
            recordCount = source_transformation_config_df.count()
            logger.info(f"Source config Record Count => {recordCount}")
            ...
            if reprocess_flag == 'Y':
                sorted_df = source_transformation_config_df.orderBy('order_seq_no')
                ...
                if sorted_list:
                    ...
                    get_ids_query = f"""SELECT {id_column_name} FROM {first_table_name} f WHERE f.source = '{source}' AND f.trading_partner IS NULL AND f.file_header_id = {file_header_id}"""
                    ...
                    if partial_bill_ids_count > 0:
                        ...
                        for row in reversed(sorted_list):
                            ...
                            delete_script = f"""DELETE t FROM {table_name} t JOIN {first_table_name} f ON f.{id_column_name}=t.{id_column_name} WHERE f.source = '{source}' AND f.trading_partner IS NULL AND f.file_header_id = {file_header_id}"""
            ...
            source_form_type_df = source_transformation_config_df.select(col("form_type")).distinct()
            source_form_type_df.printSchema()
            source_form_type_df.show(10, truncate=False)
            recordCount = source_form_type_df.count()
            logger.info(f"Source form type Record Count => {recordCount}")
            source_form_type_itr = source_form_type_df.orderBy(['form_type']).rdd.toLocalIterator()
            #Processing Trading Partner Transformation configuration in order sequence
            for frow in source_form_type_itr:
                logger.info(f'Processing started for Form Type - {frow["form_type"]}. frow: {frow}')
                source_transformation_config_itr = source_transformation_config_df.filter(source_transformation_config_df["form_type"] == frow["form_type"]).orderBy(['order_seq_no']).rdd.toLocalIterator()
                
                bill_header_table_name = ""
                #Processing Source Transformation configuration in order sequence
                for row in source_transformation_config_itr:
                    logger.info(f"Processing started for row: {row}")
                    ...
                    enhanced_df = get_df_db_data_with_query(row["db_read_sql_select"].replace('<INPUT_FILE_HEADER_ID>', str(file_header_id)), secrets)
                    ...
                    if validation_error_count > 0:
                        ...
                        join_condition = [validation_error_df['form_type'] == frow["form_type"], enhanced_df['bill_header_id'] == validation_error_df['bill_header_id']]
                    ...
                    if source == "kofax" and frow["form_type"] in ["CMS1500","UB04"] and row["table_type"] == "LINE_DETAIL":

                        logger.info(f'Starting split line logic...')

                        charge_column = "total_charge" if frow["form_type"] == "UB04" else "charges_for_services"
                        unit_column = "units_of_service" if frow["form_type"] == "UB04" else "units" 
                    ...
                    enhanced_transform_df = enhanced_transform_df \
                        .withColumn("create_user", lit("Glue ETL - Enhanced Non-ebill loading")) \
                        .withColumn("update_user", lit("Glue ETL - Enhanced Non-ebill loading")) \
                        .withColumn("create_timestamp", lit(v_load_timestamp)) \
                        .withColumn("update_timestamp", lit(v_load_timestamp))
                    enhanced_transform_df.printSchema()
                    ...
                    if row["db_select_cols_list"] or row["db_filter_id_col_name"]:
                        ...
                        db_filter_id_df = get_df_db_data_with_filter_id(source, row["db_table_name"], row["table_type"], row["db_select_cols_list"], row["db_filter_id_col_name"], v_filter_id_col_value, bill_header_table_name, secrets)
    ...
    if is_validation_errors:
        ...
        #Error Flow
        error_message = f"CustomError: Data NOT Available in Staging for Source {source}, trading partner {trading_partner} and File Header ID {file_header_id}."
        logger.error(f"error_message => {error_message}")
        raise Exception(error_message)
    else:
        status="COMPLETED"
    ...
    response=log_and_email(env_profile,errorInfo,file_header_id,None,source,trading_partner,errormessage,severity,sns_topic_arn)
    ...
    logger.info(f"db_sp_params => {db_sp_params}")
    dbStoredProcedure = "txm_bitx_enhanced_logs.update_enhanced_nonebill_log_status"
    logger.info(f"dbStoredProcedure => {dbStoredProcedure}")
    res = call_db_stored_procedure(secrets, dbStoredProcedure, db_sp_params, None)
    logger.info(f"res => {res}")
try:
