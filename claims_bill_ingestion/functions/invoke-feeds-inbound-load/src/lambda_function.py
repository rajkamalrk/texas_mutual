...
def log_and_email(env_profile, provider, feed_type, file_name, reprocess_flag, batch_id, error_info, error_message, severity, sns_topic_arn):
    """
    ...
    message_body = (
        f"Process : {provider}-{feed_type} - Invoke Feeds Inbound Load Step Function \n"
        f"Batch ID: {batch_id}\n"
        f"File Name (.completed): {file_name}\n"
    ...
    try:
        ...
        for record in event_records:
            ...
            for message in messages:
                ...
                # Extract feed type from folder structure (assumes format: .../provider/feed_type/file)
                feed_type = folder_path_list[-2].upper()
                ...
                batch_id = create_feeds_batch(secrets, feed_type, provider, file_name, folder_prefix, 'Lambda - Invoke Feeds Inbound')
                print(f"batch_id => {batch_id}")

                step_id = feeds_step_logging(secrets, batch_id, step_id, lambda_name, aws_request_id, 'IN PROGRESS', 'Lambda - Invoke Feeds Inbound')
                print(f"step_id => {step_id}")

                res_s3_feed_file_list = get_s3_objects(bucket = s3_bucket, prefix = folder_prefix + file_name_wo_ext, suffix = None)
                print(f"res_s3_feed_file_list => {res_s3_feed_file_list}")

                s3_feed_file_list = [i for i in res_s3_feed_file_list if i != s3_object_key]
                print(f"s3_feed_file_list => {s3_feed_file_list}")

                if s3_feed_file_list:
                    feed_file_name = s3_feed_file_list[0]
                else:
                    error_message = f"No feed file present in s3 bucket: {s3_bucket} and prefix: {folder_prefix + file_name_wo_ext}"
                    print(f"error_message => {error_message}")
                    raise Exception(error_message)
                ...
                update_sql = f"""UPDATE txm_inbound_feeds_logs.batch_inbound_feeds_master
                                 SET file_name = %s,
                                    reprocess_flag = %s,
                                     ...
                                     update_timestamp = %s
                                 WHERE batch_id = %s"""
                update_values = (os.path.basename(feed_file_name), 'N', invoke_response['executionArn'], 'Lambda - Invoke Feeds Inbound', update_timestamp, batch_id)
                ...
                feeds_step_logging(secrets, batch_id, step_id, None, None, status, 'Lambda - Invoke Feeds Inbound')
        ...
        error_message=str(e)
        response=log_and_email(env_profile,provider,feed_type,file_name,reprocess_flag,batch_id,error_info,error_message,severity,sns_topic_arn)
