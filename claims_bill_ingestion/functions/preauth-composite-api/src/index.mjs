...
export const handler = async (event, context) => {
  try {
    ...
    let medicalDetailsId;
    let response = await callMulesoftApi(result[0].claim_id, batchId, stepId, context, flow, result, medicalDetailsId);
    ...
    // console.log('response.count', JSON.parse(response).count);
      ...
      medicalDetailsId = medicalDetailsItem?.attributes?.id || null;
      ...
      flow = 'composite';
      let response2 = await callMulesoftApi(result[0].claim_id, batchId, stepId, context, flow, result, medicalDetailsId);
...
const callMulesoftApi = async (claimId, batchId, stepId, context, flow, result, medicalDetailsId) => {
        ...
        "requests": [
            ...
            },
            {
                ...
                "body": {
                    "data": {
                        "attributes": {
                            "body": JSON.stringify(result[0].note_body),
                            "subject": result[0].note_subject,
                            "topic": {
  ...
  try {
    ...
    await logErrorResponse(error,batchId, stepId, context);
...
const logErrorResponse = async (error, batchId, stepId, context) => {
