...
export const handler = async (event) => {
    ...
    try {
        ...
        [result] = await connection.query(`
          ...
          WHERE 
            claim_number = ? and header_id = ? and batch_id = ?
        `, [claimNumber, headerId, batchId]);
        ...
        if (result.length > 0){
            for (let i = 0; i < result.length; i++){
                try{
                    ...
                    if (!result[i].document_serial_number){
                        const response = await sendAttachmentToNuxeo(result[i]);
                        console.log('Main Response', response);
                        if (response.documentSerialNumber){
                            console.log("update bill document serial number");
                            const updateQueryBT = `UPDATE ${process.env.DB_1}.${process.env.TABLE_1} SET  update_user = 'LAMBDA', update_timestamp = ?, document_serial_number  = ?, nuxeo_status = ? WHERE claim_number = ? and header_id = ? and batch_id = ?`;
                            const updateValuesBT = [updatedOn, response.documentSerialNumber, 'COMPLETED', result[i].claim_number, result[i].header_id, result[i].batch_id];
                    ...
                    const updateQueryBT = `UPDATE ${process.env.DB_1}.${process.env.TABLE_1} SET  update_user = 'LAMBDA', update_timestamp = ?, nuxeo_status = ? WHERE claim_number = ? and header_id = ? and batch_id = ?`;
                    const updateValuesBT = [updatedOn, 'FAILED', result[i].claim_number, result[i].header_id, result[i].batch_id];
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                    const insertValues = [batchId, stepId, "LAMBDA FAILED", "Failed while sending request to NUXEO", JSON.stringify(error), createdOn, createdOn, 'LAMBDA', 'LAMBDA'];
                    await connection.beginTransaction();
                    await connection.query(updateQueryBT, updateValuesBT);
                    await connection.query(insertQuery, insertValues);
                    await connection.commit();
                    // const ts = new Date().toLocaleString('en-US',{timeZone:'America/Chicago'});
                    // const subject = `${process.env.ENVIRONMENT} - ${tp} - Failed to send pre auth letters request to NUXEO - FATAL`;
                    // const message = `Process: ${tp} - Send attachment pre auth letters request to NUXEO \n Error: ${error} \n Timestamp: ${ts}` ;
            ...
            return {
                statusCode: 200,
                body: { message: "No record found for given claim number combination"},
            };
        }
...
async function sendAttachmentToNuxeo(result) {
    
    console.log('In sendAttachmentToNuxeo', result);
    
    let utcDate = result.receive_date;
    let date = new Date(utcDate);
    date.setHours(date.getHours() + 6);
    let cstDate = date.toISOString();
    let docItemType = result.txm_document_type;
    console.log(cstDate);
    let nuxeoPayload = {
        "receiveDate": cstDate,
        "filePath": result.document_path,
        "itemType": docItemType,
        "sourceId": result.document_name,
        "sourceSystem": "GENEX",
