...
const fetchSecretDetails = async () => {
  ...
  try {
    const response = await secretsManagerClient.send(new GetSecretValueCommand({ SecretId: secretName, VersionStage: "AWSCURRENT" }));
    return JSON.parse(response.SecretString);
  } catch (error) {
    console.error('Error fetching secret details:', error);
    throw new Error('Could not fetch secret details');
  }
};

const createDbPool = (secretDetails) => {
...
const validateRequestFields = (requestFields, context) => {
  const { claimNumber, vendorNumber } = requestFields;
  ...
  if (vendorNumber && (typeof vendorNumber !== 'string' || vendorNumber.trim() === '')) {
    return generateErrorResponse(context.awsRequestId, 'Invalid Payload', 'Vendor Number is missing or it is invalid', 400);
  }
  if (!claimNumber && !vendorNumber) {
    return generateErrorResponse(context.awsRequestId, 'Invalid Payload', 'Either claimNumber or vendorNumber is missing or it is invalid', 400);
  }
  return null;
};

const updateMatchResponse = async (pool, tableName, invoiceNumber, matchFound, statusType, responseValue, context) => {
  ...
  try {
    ...
    if (statusType === 'CLAIM_MATCH_MANUAL' && responseValue !== 'DELETE'){
      ...
      dbField = 'txm_claim_number';
      responseField = responseValue;
      updateQuery = `UPDATE ${process.env.DB_1}.${responseTable} SET ${dbField} = ?, update_user = 'LAMBDA', update_timestamp = ? WHERE txm_invoice_number = ?`
      queryValue = [responseField, createdOn, invoiceNumber];
    }
      ...
      dbField = 'vendor_number';
      responseField = responseValue;
      updateQuery = `UPDATE ${process.env.DB_1}.${responseTable} SET ${dbField} = ?, update_user = 'LAMBDA', update_timestamp = ? WHERE bill_id = ?`;
      queryValue = [responseField, createdOn, rowsValidate[0].bill_id];
    }
    else if (statusType === 'VENDOR_MATCH_MANUAL' && responseValue === 'DELETE'){
      responseTable = process.env.TABLE_4;
      dbField = 'bill_id';
    ...
    await connection.query(`UPDATE ${process.env.DB_1}.${process.env.TABLE_2} SET status = ?, update_user = 'LAMBDA', update_timestamp = ? WHERE txm_invoice_number = ? AND status_type = ?`, [matchFound, updatedOn, invoiceNumber, statusType]);
    
    const [rows2] = await connection.query(`SELECT status_id FROM ${process.env.DB_1}.${process.env.TABLE_2} WHERE txm_invoice_number = ? AND status_type = ?`, [invoiceNumber, statusType]);
...
export const handler = async (event, context) => {
  ...
  try {
    if (event.httpMethod === 'PUT') {
      ...
      const secretDetails = await fetchSecretDetails();
      const pool = createDbPool(secretDetails);
      result = await updateMatchResponse(pool, process.env.Table2, invoiceNumber, matchFound, 'CLAIM_MATCH_MANUAL', 'DELETE', context);
    ...
    if (claimNumber && !vendorNumber) {
      result = await updateMatchResponse(pool, process.env.TABLE_2, invoiceNumber, matchFound, 'CLAIM_MATCH_MANUAL', claimNumber, context);
      responseType = 'claim match';
    } else if (!claimNumber && vendorNumber) {
      result = await updateMatchResponse(pool, process.env.TABLE_2, invoiceNumber, matchFound, 'VENDOR_MATCH_MANUAL', vendorNumber, context);
      responseType = 'vendor match';
    }
