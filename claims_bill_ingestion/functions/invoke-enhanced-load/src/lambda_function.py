...
def log_and_email(env_profile,source,trading_partner,file_header_id,reprocess_flag,sf_batch_id,error_info,error_message,severity,sns_topic_arn):
    ...
    #create log message
    process=f"Process : {source_tp_str} - Invoke Enhanced Load Step Function"
...
def lambda_handler(event, context):
    try:
        ...
        for record in event_records:
            ...
            file_header_id = message.get('file_header_id')
            reprocess_flag = message.get('reprocess_flag', 'N').upper()  # Default to N if not present
            sf_batch_id = message.get('sf_batch_id')

            if not sf_batch_id:
                db_sp_params = source, trading_partner, file_header_id, None, None, json.dumps(message), None, None, None ,None, None, None, 'INVOKE ENHANCED', reprocess_flag,  'Lambda - Invoke Enhanced', 'Lambda - Invoke Enhanced', 0
                print(f"db_sp_params => {db_sp_params}")
            ...
            #updated log table
            db_sp_params = sf_batch_id, None, None, None, None, None ,None, invoke_response['executionArn'], json.dumps(payload), None, None, 'ENHANCED', 'IN PROGRESS',  'Lambda - Invoke Enhanced'
            print(f"db_sp_params => {db_sp_params}")
        ...
        severity="FATAL"
        errorInfo="Unable to invoke Enhanced Load Step Function"
        ...
        if sf_batch_id:
            #updated log table
            db_sp_params = sf_batch_id, 'LAMBDA', lambda_name, aws_request_id, json.dumps(payload), 'FAILED', 'INVOKE ENHANCED LOAD', errorInfo, errorMessage, 'Lambda - Invoke Enhanced', 'Lambda - Invoke Enhanced'
            print(f"db_sp_params => {db_sp_params}")
