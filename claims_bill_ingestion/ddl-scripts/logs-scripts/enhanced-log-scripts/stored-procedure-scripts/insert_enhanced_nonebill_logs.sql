DROP PROCEDURE IF EXISTS txm_bitx_enhanced_logs.insert_enhanced_nonebill_logs;

DELIMITER $$
CREATE PROCEDURE txm_bitx_enhanced_logs.insert_enhanced_nonebill_logs(
...
BEGIN
	...
	SET message = CONCAT('Inserting to enhanced logs db table txm_bitx_enhanced_logs.batch_non_ebill_master...');
    SELECT message AS log_info;
        INSERT INTO txm_bitx_enhanced_logs.batch_non_ebill_master (source
													,trading_partner
	                                                ,file_header_id
													,data_pipeline_id
													,job_id
