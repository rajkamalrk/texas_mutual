DROP PROCEDURE IF EXISTS txm_bitx_enhanced_logs.insert_enhanced_logs;

DELIMITER $$
CREATE PROCEDURE txm_bitx_enhanced_logs.insert_enhanced_logs (
...
BEGIN
	...
	SET message = CONCAT('Inserting to staging logs db table tmi_bitx_enhanced_logs.batch_enhanced_master...');
    SELECT message AS log_info;
        INSERT INTO txm_bitx_enhanced_logs.batch_enhanced_master (trading_partner
	                                                ,file_header_id
													,data_pipeline_id
													,job_id
													,status
