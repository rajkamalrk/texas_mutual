DROP PROCEDURE IF EXISTS txm_bitx_staging_logs.update_staging_log_status;

DELIMITER $$
CREATE PROCEDURE txm_bitx_staging_logs.update_staging_log_status (
    IN p_batch_id INT,
...
BEGIN
	...
	SET message = CONCAT('Updating staging logs db table txm_bitx_staging_logs.batch_staging_details for BATCH_ID::', p_batch_id, '...');
    SELECT message AS log_info;
    UPDATE txm_bitx_staging_logs.batch_staging_details
    ...
    THEN
	  SET message = CONCAT('Inserting to staging logs db table txm_bitx_staging_logs.batch_staging_error_details...');
      SELECT message AS log_info;
      
      INSERT INTO txm_bitx_staging_logs.batch_staging_error_details (batch_id
														    ,error_code
														    ,error_type
														    ,error_message
														    ,create_timestamp
