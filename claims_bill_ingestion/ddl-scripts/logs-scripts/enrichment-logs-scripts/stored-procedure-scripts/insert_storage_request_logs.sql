DROP PROCEDURE IF EXISTS txm_bitx_enrichment_logs.insert_storage_request_logs;
DELIMITER $$
CREATE PROCEDURE txm_bitx_enrichment_logs.insert_storage_request_logs(
...
BEGIN
	...
	SET message = CONCAT('Inserting to enrichment logs db table txm_bitx_enrichment_logs.batch_storage_request_master...');
    SELECT message AS log_info;
        INSERT INTO txm_bitx_enrichment_logs.batch_storage_request_master (source
		                                            ,trading_partner
	                                                ,file_header_id
													,data_pipeline_id
													,job_id
