DROP PROCEDURE IF EXISTS txm_bitx_enrichment_logs.update_claim_match_log_status;
DELIMITER $$
CREATE PROCEDURE txm_bitx_enrichment_logs.update_claim_match_log_status(
    IN p_batch_id INT,
    IN p_bill_id INT,
...
BEGIN
	...
	SET message = CONCAT('Updating enrichment logs db table txm_bitx_enrichment_logs.batch_claim_match_master for BATCH_ID::', p_batch_id, '...');
    SELECT message AS log_info;
    UPDATE txm_bitx_enrichment_logs.batch_claim_match_master
    ...
    THEN
	  SET message = CONCAT('Inserting to enrichment logs db table txm_bitx_enrichment_logs.batch_claim_match_error_detail...');
      SELECT message AS log_info;
      
      INSERT INTO txm_bitx_enrichment_logs.batch_claim_match_error_details (batch_id
														    ,bill_id
                                                            ,txm_invoice_number
                                                            ,error_code
														    ,error_type
